...
class RAGSystem:
    ...
    def enhance_prompt_with_references(self, board_state: Dict[str, any], base_prompt: str) -> tuple:
        """
        Enhance AI prompt and return both the enhanced prompt and the RAG references used.
        ...
        Returns:
            Tuple of (enhanced_prompt, references_dict)
        """
        from concurrent.futures import ThreadPoolExecutor, as_completed
        ...
        with ThreadPoolExecutor() as executor:
            ...
            card_names_to_query = self._get_card_names_from_board(board_state)
            card_names_queried = card_names_to_query  # Track what was queried
            for card_name in card_names_to_query:
                tasks.append(executor.submit(self.get_card_stats, card_name))
        ...
        # 4. Create references dictionary for UI display
        references = {
            "rules": rules_results if rules_results else [],
            "card_stats": stats_results if stats_results else {},
            "situation_queries": situation_queries,
            "cards_analyzed": card_names_queried,
            "phase": phase
        }
