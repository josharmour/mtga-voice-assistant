...
from .coach import CoachEngine, GameStateTrigger, create_backend
...
from .draftstats import DraftStatsCache
from .draftstate import DraftState, create_draft_handler
from .draft_eval import evaluate_pack, format_pick_recommendation
from .sealed_eval import analyze_sealed_pool, format_sealed_recommendation, format_sealed_detailed
from .mtgadb import MTGADatabase
from .mtgjson import MTGJSONDatabase, get_mtgjson
...
def _get_mtggoldfish():
    ...
    if _mtggoldfish is None:
        from .mtggoldfish import MTGGoldfishClient
...
def _get_deck_builder():
    ...
    if _deck_builder is None:
        from .deck_builder import DeckBuilderV2
...
def _get_synergy_graph():
    ...
    if _synergy_graph is None:
        from .synergy import get_synergy_graph
...
def _handle_match_created(payload: dict) -> None:
    ...
    if seat_id is None:
        ...
        from .log_utils import get_local_player_id
        ...
        if seat_id is None and not local_user_id:
             ...
             from .log_utils import detect_local_seat
