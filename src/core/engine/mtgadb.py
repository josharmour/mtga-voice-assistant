...
class MTGADatabase:
    ...
    def __init__(self, db_path: Optional[Path] = None):
        ...
        if self._db_path and self._db_path.exists():
            try:
                ...
                # Enable shared cache for performance
    ...
    def _resolve_oracle_text(self, ability_ids_str: Optional[str]) -> str:
        ...
        try:
            # Parse 'AbilityGrpId:TextId' pairs
            text_ids = []
            for pair in ability_ids_str.split(','):
                parts = pair.split(':')
                if len(parts) >= 2:
                    text_ids.append(parts[1])
            
            if not text_ids:
                return ""
                
            # Query all text IDs at once
            placeholders = ",".join("?" * len(text_ids))
            cursor = self._conn.execute(f"""
                SELECT Loc FROM Localizations_enUS 
                WHERE LocId IN ({placeholders})
                ORDER BY CASE LocId 
                    {' '.join(f'WHEN {tid} THEN {i}' for i, tid in enumerate(text_ids))}
                END
            """, text_ids)
